...
## The obstacle, named — EyeSight's echo cross-check (added 2026-08-06, after archaeology)

A parallel research pass recovered first-hand community accounts that both **confirm the
core hypothesis** and **name the specific thing that defeated two previous attempts**. The
quotes below were spot-checked verbatim against the raw local Discord export
(`discord-logs/CommaDiscord/comma.ai community - Vehicle Specific - subaru [...].txt`),
not taken second-hand — this project has been burned by fabricated quotes before. Full
context: `research/preglobal_longitudinal_command_archaeology.md`.

**Confirmation, from the person who reverse-engineered this platform's bus** — an exchange
dated 10/12/2020, on preglobal:

> **aileron.me:** So both ES_CruiseThrottle.Throttle_Cruise and ES_RPM are generated by ES
> and consumed by ECU?
>
> **bugsyborromeo:** yes

Also in the same exchange: `ES_RPM` *"controls engine rpm"*, and *"ES_RPM's request traces
with the transmission message on the main bus."* This is direct human testimony that
`0x161`'s throttle field is an **EyeSight-generated command consumed by the engine ECU** on
preglobal — the exact claim this document's Stage 0 was designed to test. It is testimony,
not data, but it is first-hand and from the right person.

**The obstacle, same conversation:**

> **bugsyborromeo:** not that simple, ES will error if you set it to 0 and it doesn't hear
> back the same thing from 0x140

**EyeSight validates its own commanded throttle against the engine's echo in `Throttle`
(`0x140`) `Throttle_Cruise`, and faults on mismatch.** Note that `0x161`'s throttle field
was itself originally *named* `Throttle_Cruise` (see §1b) — command and echo, same name.

This is why both known preglobal attempts failed, and it applies to the architecture in
this document too: EyeSight commands X on the camera bus, we transmit Y on the main bus,
the engine echoes Y in `0x140`, EyeSight sees Y ≠ X. Being a byte-faithful relay rather
than a wholesale replacement does not dodge it.

**Why this is not yet a kill, stated carefully:**

- **Nobody has tested the tolerance.** Both prior attempts commanded *maximal* divergence —
  literally `0`, with EyeSight's messages blocked and substitutes generated from hardcoded
  fields. The question "does EyeSight fault on a 5% deviation held for 200ms" has never
  been asked by anyone. The cross-check's tolerance band and time constant are unknown.
- **The tolerance is measurable from the archive, for free.** How tightly `0x140`'s
  `Throttle_Cruise` normally tracks `0x161`'s `Cruise_Throttle`, at what lag, with what
  residual — and critically, **what the echo does during a driver gas override**, when the
  engine is manifestly not doing what EyeSight asked. If the echo follows the pedal, then
  EyeSight already tolerates large mismatches routinely and the cross-check is far weaker
  than a fault-on-zeros result suggests.
- **The echo is writable, with no firmware change.** `Throttle` (`0x140`) is a car→camera
  message and sunnypilot's stop-and-go **already TX-allows it on the camera bus**,
  reachable today via the `SubaruStopAndGo` param. Feeding EyeSight the echo it expects
  closes exactly the loop that broke both prior attempts. Neither of them tried this —
  bugsy's fwd hook blocks camera→car addresses and leaves `0x140` car→camera untouched.
  It is messy (`check_relay = false`, so real frames still interleave) but it is a real,
  precedented path.

**Consequence for Stage 0: measure the echo relationship first**, before the
command-vs-report question. It determines whether there is a tolerance band to work inside
at all, and therefore whether the `0x140` echo-spoof is a fallback or a day-one
requirement.

**One more warning this surfaced, about the test design itself:** a naive "if `Cruise_Throttle`
tightly matches a powertrain report at some lag, it's a report" rule would score the ECU's
*acknowledgement of a genuine command* as proof the field is an echo — a false kill on the
strongest possible confirmation. The kill criterion has to be fitted separately either side
of zero lag and only fire when the echo-side fit is both good enough *and better than* the
command-side fit. The same trap applies to `Cruise_RPM` vs `Transmission_Engine`, which
bugsy's *"traces with the transmission message"* line describes exactly.
