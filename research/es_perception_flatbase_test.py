#!/usr/bin/env python3
"""
Test A, pre-registered in research/es_stage0_prereg_round2.md, run here EXACTLY
as specified there -- do not tune anything in this file to change the outcome.

WHY THIS EXISTS: es_perception_focus.py (the round-1 perception-trigger test)
came back INCONCLUSIVE. Its own traces showed why: Cruise_Throttle was already
declining ~300ms BEFORE the Car_Follow flag flipped, because the flag is a
thresholded readout of a continuous internal estimate, not an exogenous
impulse. That also contaminated the onset detector's own baseline window with
the pre-flip response.

THE FIX (pre-registered, not tuned post-hoc): only score events where
Cruise_Throttle is PROVABLY FLAT through the entire baseline window
([-1000ms, -50ms], peak-to-peak <= 30 counts -- derived from the existing
40-count onset threshold, so a compliant baseline by construction contains
nothing the detector would itself call an onset). This selects for genuine
surprises (an abrupt cut-in) instead of gradual overtakes EyeSight has already
been reacting to for seconds, which simultaneously restores a real impulse and
decontaminates the baseline.

PRIMARY ENDPOINT (fixed in the pre-registration, applied mechanically here):
  frac_ct_first_50 = fraction of qualifying LEAD-ACQUIRED events where
  onset(Cruise_Throttle) precedes onset(Throttle_Body) by >= 50ms.

  CONFIRM: frac_ct_first_50 >= 70% AND median margin >= 75ms
  NULL:    frac_ct_first_50 <= 50% OR median margin <= 25ms
  INCONCLUSIVE: anything between, or N < 100 (the power floor)

engine_rpm_140 is EXCLUDED from the primary endpoint by pre-registration (it is
slow and CVT-decoupled -- throttle_body beats it by ~280ms too in round 1, so a
large margin against it carries no command-vs-report information). Reported as
descriptive only. wheel_torque is also descriptive only, not primary.
Lead-LOST events are excluded from the primary set (round 1: slower, messier)
and reported as an unfiltered secondary for continuity with round 1, not
subject to the flat-baseline filter.

The ~25ms quantization bias (ES_Distance is 20Hz, so Cruise_Throttle's onset
is on average ~25ms late relative to truth) is acknowledged and NOT corrected
for -- it biases AGAINST finding a CT lead, i.e. against the hypothesis, so
"fixing" it would only be tunable in the direction of a false CONFIRM.
...
OUT_FILE = Path("/work/es_perception_flatbase_results.json")
...
FLATBASE_MAX_PP2P = 30.0   # Cruise_Throttle peak-to-peak in baseline must be <= this
REAL_RESPONSE_MIN = 40.0   # |delta Cruise_Throttle| required somewhere in (0, POST_NS]
...
# identical to every prior run -- not retuned for this test
ONSET_THRESHOLDS = {
...
def classify_and_record(acc, bufs, t, route_id, direction, per_route):
  ...
  for pw in POWERTRAIN:
    ...
    if ct is None or pwl is None:
      ...
      ev[f"margin_vs_{pw}_ms"] = pwl - ct  # positive => Cruise_Throttle moved FIRST
...
def process_route(log_schema, acc, route_id, segments):
  ...
  def drain(now, force=False):
    while pending and (force or now - pending[0] > SETTLE_NS):
      ...
      ct_win_full = bufs.window("es_cruise_throttle", t - GATE_PRE_NS, t + GATE_POST_NS)
      if ct_win_full and max(v for _, v in ct_win_full) <= 810:
        acc.rejected["ct_pinned_at_floor"] += 1
        continue

      # --- PRE-REGISTERED FLAT-BASELINE FILTER, primary (acquired) set only ---
      if direction == "acquired":
        base_win = bufs.window("es_cruise_throttle", t - PRE_NS, t - 50 * MS)
        if len(base_win) < 5:
        ...
        post_win = bufs.window("es_cruise_throttle", t, t + POST_NS)
...
def apply_prereg_verdict(acquired_summary, n_qualifying):
  ...
  tb = acquired_summary["vs_throttle_body"]
  frac50 = tb["frac_cruise_throttle_first_by_50ms_or_more"]
  median_margin = tb["median_margin_ms"]

  if n_qualifying < POWER_FLOOR_N:
    return {
      "verdict": "INCONCLUSIVE",
      "reason": f"n_qualifying={n_qualifying} < power floor {POWER_FLOOR_N}",
      "frac_ct_first_50": frac50, "median_margin_ms": median_margin,
    }
  ...
  return {
    "verdict": verdict,
    "reason": f"frac_ct_first_50={frac50}%, median_margin={median_margin}ms, n={n_qualifying}",
    "frac_ct_first_50": frac50, "median_margin_ms": median_margin,
  }
